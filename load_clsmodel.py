...
class Pretrained_model(nn.Module):
    def __init__(self, decoder_mode, finetune_gpt2, img_feature_h, img_feature_w):
        ...
        self.img_feature_h = img_feature_h  #7
        self.img_feature_w = img_feature_w

        self.finetune_gpt2 = finetune_gpt2  #false
        ...
        # position embedding：
        l = 50
        self.l_embedding = nn.Embedding(l, int(self.d_model))  #50,768
    ...
    def Siamese_bridge_net(self, class_embedding, img_feat):
        ...
        cls_A = conc_A[:, 0, :]
        #self.cls_projection(conc_A[:, 0, :])  这里是论文里说的，将cls_A过一个linear和softmax，得到pcls（见第6页上面的图）
    ...
    def Classifier(self, clip_emb, img_feat, imgid = None):
        ...
        _, img_feat = self.position_embedding_2D_func(img_feat_A, img_feat)  # NLD  #这里为什么还要pos_embeding?因为减没了吗?而且这个img_feat后续没有用上啊

        img_feat = torch.cat([img_feat_A, img_feat_B], dim=-1)  #[B,49,768*2]
        conc_A = torch.cat(
            [self.class_embedding_classifier_changeflag.unsqueeze(0).expand(img_feat.shape[0],
             ...
             img_feat], dim=1)  #[1,1536]=>[B,1,1536]=>[B,50,1536]
