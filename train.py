...
def train(args, train_loader, Caption_model, Caption_model_optimizer,Caption_model_lr_scheduler, epoch):
    ...
    for idx, (ori_img, changeflag, caps, mask, caplens, imgid) in enumerate(tqdm(train_loader, desc=f'Epoch [{epoch+1}/{args.epochs}]', unit='batch')):
        ...
        cap_top5 = accuracy(scores, targets, 5)
        ...
        if idx % args.print_freq == 0:
            tqdm.write("Epoch:{}/{} step:{}/{} Loss:{:.4f} AVG_Loss:{:.4f} CAP_Loss:{:.4f} CAP_AVG_Loss:{:.4f} Flag_Loss:{:.4f} Flag_AVG_Loss:{:.4f} CAP_Acc:{:.2f} Flag_Acc:{:.2f} Batch_time:{:.2f}s"
...
def main(args):
    ...
    if args.dataset_name == 'LEVIR_CC':  #2*7*7 + 5 + 1
    ...
    train_loader = torch.utils.data.DataLoader(
        CaptionDataset(args.clip_model_type, args.dataset_name, args.data_folder, args.data_name, 'TRAIN',
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        save_checkpoint(args, checkpoint_name, epoch, epochs_since_improvement, Caption_model,
                       Caption_model_optimizer, metrics, True)
...
if __name__ == '__main__':
    ...
    # Training parameters
    parser.add_argument('--epochs', type=int, default=2, help='number of epochs to train for (if early stopping is not triggered).')
